  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 sm:py-16">
      ...
      <p className="text-muted mt-5 text-lg leading-relaxed">
        A revenue figure is only worth reading if you know where it came from. Here is exactly what
        happens between a founder&apos;s payment provider and the number on their profile —
      ...
      <Section title="1. The founder connects a read-only key">
        ...
        <div className="mt-5 space-y-4">
          {CONNECTABLE_PROVIDERS.map((provider) => (
            <div key={provider.id} className="border-border bg-surface rounded-[10px] border p-5">
      ...
      <Section title="2. The key is encrypted before it reaches the database">
        <p>
          ...
          written to a table that no browser session can read. The founders who add them cannot read
          them back either — reconnecting always means entering the credential again.
        </p>
      </Section>
      ...
      <Section title="What this does not prove">
        <p>
          Verification confirms that a provider account reports this revenue. It does not audit the
          business behind it. Specifically, it cannot tell you:
        </p>
      ...
      <Section title="Superwall">
        <p>
          ...
          publishes no API for reading revenue charts, so there is no way for us to verify a figure
          through it. We would rather list nothing than a badge we cannot stand behind. If that
          changes, we will add it.
        </p>
      </Section>

      <div className="border-border bg-surface mt-12 rounded-[10px] border p-6">
