...
export function SubmitFlow({
  ...
  const suggested = app.primaryGenre
    ? categories.find((category) => category.genre === app.primaryGenre)
    : undefined
    ...
    <form action={runSubmit} className="mt-8 space-y-6">
      ...
      {submit.needsAuth && (
        <div className="border-border bg-surface rounded-[10px] border p-4">
          <h2 className="text-fg flex items-center gap-2 text-[13px] font-bold">
            ...
            One step left — sign in to save
          </h2>
          <p className="text-muted mt-1.5 text-[12px] leading-relaxed">
            A listing belongs to a founder, so we need to know who you are before saving. Your app
            details come straight back after you sign in.
          </p>
          <ButtonLink
            href={`/login?next=${encodeURIComponent(`/submit?id=${submit.needsAuth.appStoreId}`)}`}
            className="mt-3"
          >
            Sign in and save
          </ButtonLink>
      ...
      <div className="border-border flex items-center gap-3 border-t pt-6">
        ...
        <p className="text-muted text-xs">
          {isSignedIn
            ? 'Saved as a private draft until revenue verifies.'
            : "You'll sign in to save. Nothing is published until revenue verifies."}
        </p>
      </div>
    </form>
  )
